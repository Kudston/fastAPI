...
def create_user(db: Session, user: blogapi_schemas.user_create_class):
    ...
    db_counts = len(db.query(models.User_model).all())
    db_user = models.User_model(id=db_counts, username=user.username, email=user.email, hashed_password=hashed_password, created_on=current_time, updated_on=current_time)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_users_blog(db: Session, blog: blogapi_schemas.Blog_Schema, user_id: int):
    current_time = datetime.utcnow()
    db_blog = models.Blog(**blog.dict(),author_id=user_id, created_on=current_time, updated_on=current_time)
    db.add(db_blog)
    db.commit()
    db.refresh(db_blog)
    return db_blog
...
def update_blog(db:Session, update_info:blogapi_schemas.Blog_Schema, user_id:int, blog_id:int):
    user_blog_query = get_user_blog(db, user_id=user_id, blog_id=blog_id)
